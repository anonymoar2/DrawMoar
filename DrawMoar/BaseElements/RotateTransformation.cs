...
namespace DrawMoar.BaseElements
{
    class RotateTransformation : Transformation
    {
        ...
        public RotateTransformation(System.Windows.Point point, double angle) {
            this.angle = angle;
            var one = new Matrix<double>(new double[3, 3] {
                { 1, 0, point.X },
                { 0, 1, point.Y },
                { 0, 0, 1 }
            });
            var three = new Matrix<double>(new double[3, 3] {
                { 1, 0, -point.X },
                { 0, 1, -point.Y },
                { 0, 0, 1 }
            });
            var two = new Matrix<double>(new double[3, 3] {
        ...
        public override Picture Apply(Picture picture) {
            ...
            gfx.DrawImage(picture.Image, new Point(0, 0));
            gfx.Dispose();
            return picture;
        }

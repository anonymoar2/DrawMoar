...
namespace DrawMoar.BaseElements
{
    ...
    public class Transformation
    {
        ...
        public static Transformation Translate(System.Windows.Point point) {
            return new Transformation(new Matrix<double>(new double[3, 3] {{1,0,point.X},{0,1,point.Y},{0,0,1}}));
        }
        public static Transformation Scale(System.Windows.Point point, double scaleFactor) {
            var trans = Translate(point);
            var tI = Translate(new System.Windows.Point(-point.X, -point.Y));
            var s = new Transformation(new Matrix<double>(new double[3,3] {{scaleFactor,0,0},{ 0,scaleFactor,0},{0,0,1}}));
            return trans * s * tI;
        }
        public static Transformation Scale(System.Windows.Point point1, System.Windows.Point point2, double scaleFactor) {
            var t = Translate(point1);
            var tI = Translate(new System.Windows.Point(-point1.X, -point1.Y));
            var tP = tI[point2];
            var a = RadToDeg(Math.Asin(tP.Y/Math.Sqrt(tP.X * tP.X+tP.Y*tP.Y)));
            var r = Rotate(new System.Windows.Point(), -a);
            var rI = Rotate(new System.Windows.Point(), a);
            var s = new Transformation(new Matrix<double>(new double[3, 3] {{ scaleFactor,0,0},{0,1,0},{0,0,1}}));
            return t * r * s * rI * tI;
        }
        public void Decompose(out System.Windows.Point translation, out System.Windows.Point scale, out double rotation) {
            ...
            scale = new System.Windows.Point(xS, yS);
        }

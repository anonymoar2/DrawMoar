...
**Read it:**
    ...
    а их имена должны быть "imgТУТЗВЁЗДОЧКА.png", где ТУТЗВЁЗДОЧКА это любой символ или несколько символов (наверное), и они должны
	находиться в папке в Demo/bin/Debug/ТУТИМЯПАПКИ, имя папки указывается в двух методах CreateConcatFile() и Save() в классе
	Mp4Exporter. Это пока так чтобы было легче тестировать эту часть. Потом исправлю когда появится внутреннее представление и сами
	Frame и Cartoon и Layer, можете исправить сами.
